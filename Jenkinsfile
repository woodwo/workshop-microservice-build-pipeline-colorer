def srcVersion = ''
def branchQual = ''
def dockerImageBase = 'nexus.build.svc.cluster.local:5000/workshop/colorer'
def dockerImageTag = ''
                ...
                stage('deploy') {
                    container ('helm') {
                        withCredentials([
                            ...
                            sh """
                                kubectl delete secrets registry-colorer --kubeconfig='${k8sConfig}' --namespace '${namespace}' || true

                                kubectl create secret docker-registry registry-colorer \\
